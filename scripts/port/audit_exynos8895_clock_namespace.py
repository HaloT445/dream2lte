#!/usr/bin/env python3
"""Verify that the Exynos8895 vendor clock ABI is isolated from Linux 4.19."""
...
for original, namespaced in RENAMES.items():
    ...
    if namespaced not in vendor_c or namespaced not in vendor_h:
        errors.append(f"missing namespaced vendor declaration/definition: {namespaced}")
...
for symbol in (
    ...
    if not definition_present(core_pll, symbol):
        errors.append(f"Linux 4.19 core PLL symbol was removed: {symbol}")
...
print(f"Exynos8895 clock namespace audit: PASS ({len(RENAMES)} collisions isolated)")
